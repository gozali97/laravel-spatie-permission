    ...
    <div class="modal fade text-left" id="insertModal" tabindex="-1" aria-labelledby="insertModal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('roles.addPermission') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <input type="hidden" name="role_id" value="{{ $role->id }}">
                        <label for="nama">Nama Permission: </label>
                        <fieldset class="form-group mt-1">
                            <select name="permission" class="form-select" id="basicSelect">
                                <option value="">-- Pilih --</option>
                                @foreach ($permission as $p)
                                    <option value="{{ $p->id }}">
                                        {{ $p->name }}</option>
        ...
        function confirmDelete(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = "{{ url('/konfigurasi/roles/removePermission') }}/" + id + "/" +
                        role_id;
                }
            });
        }
